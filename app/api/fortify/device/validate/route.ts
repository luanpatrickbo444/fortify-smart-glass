...
import { allowedDevices, SESSION_TTL_SECONDS } from "@/lib/config";
...
export async function POST(request: Request) {
  ...
  const token = await signToken({ sub: mfa.sub, stage: "authenticated", deviceId, permissions }, SESSION_TTL_SECONDS);
  const response = NextResponse.json({ ok: true, sessionEstablished: true, expiresIn: SESSION_TTL_SECONDS }, { headers: { "Cache-Control": "no-store, private" } });
  response.cookies.set(COOKIE_SESSION, token, authCookieOptions(SESSION_TTL_SECONDS));
